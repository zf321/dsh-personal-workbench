...
export function makeIdeaRoutes(db) {
    return [
        {
            kind: 'prefix',
            path: IDEAS_PREFIX,
            handler: async (req, res) => {
                ...
                if (segments.length === 0) {
                    if (method === 'GET') {
                        ...
                        if (kindCode !== undefined && getDictionary(db, 'idea_kind', kindCode) === undefined)
                            return writeJson(res, 400, { error: 'unknown idea_kind' });
                ...
                if (method === 'PATCH' && segments.length === 1) {
                    ...
                    if (typeof body.kindCode === 'string') {
                        requireCode(db, 'idea_kind', body.kindCode, 'kindCode');
                    ...
                    if (idea === undefined)
                        return writeJson(res, 404, { error: 'idea not found' });

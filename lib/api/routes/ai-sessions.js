...
export function makeAiSessionRoutes(db) {
    return [
        {
            kind: 'prefix',
            ...
            handler: async (req, res) => {
                ...
                if (method === 'GET') {
                    ...
                    if (scopeCode === null || scopeCode === '')
                        return writeJson(res, 400, { error: 'scope_code is required' });
                    if (anchor === null || anchor === '')
                        return writeJson(res, 400, { error: 'anchor is required' });
                    requireCode(db, 'ai_session_scope', scopeCode, 'scope_code');
                    if (['daily_plan', 'day_report', 'week_report'].includes(scopeCode) && !PERIOD_DATE_RE.test(anchor))
                        return writeJson(res, 400, { error: 'anchor must be YYYY-MM-DD' });
                ...
                if (method === 'POST') {
                    ...
                    if (scopeCode === undefined || anchor === undefined || anchor === '' || sessionId === undefined || sessionId.trim() === '')
                        return writeJson(res, 400, { error: 'scopeCode, anchor and sessionId are required' });
                    requireCode(db, 'ai_session_scope', scopeCode, 'scope_code');
                    if (['daily_plan', 'day_report', 'week_report'].includes(scopeCode) && !PERIOD_DATE_RE.test(anchor))
                        return writeJson(res, 400, { error: 'anchor must be YYYY-MM-DD' });

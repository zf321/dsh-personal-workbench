...
export function makeReminderRoutes(db, deps = {}) {
    return [
        {
            kind: 'exact',
            ...
            handler: async (req, res) => {
                ...
                if (deps.policy === undefined)
                    return writeJson(res, 503, { error: 'reminder policy unavailable' });
                const method = req.method ?? 'GET';
                if (method === 'GET')
                    return writeJson(res, 200, { ok: true, policy: deps.policy.read() });
                if (method === 'POST') {
        ...
        },
        {
            ...
            handler: async (req, res) => {
                ...
                if (deps.channel === undefined)
                    return writeJson(res, 503, { error: 'reminder channel unavailable' });
                const method = req.method ?? 'GET';
                if (method === 'GET') {
                ...
                if (method === 'POST') {
                    ...
                    if ('botId' in body)
                        writeMeta(db, 'reminder_bot_id', typeof body.botId === 'string' ? body.botId : '');
                    if ('targetId' in body)
                        writeMeta(db, 'reminder_target_id', typeof body.targetId === 'string' ? body.targetId : '');
        ...
        },
        {
            ...
            handler: async (req, res) => {
                ...
                if (segments.length === 2 && segments[1] === 'fire' && method === 'POST') {
                    if (deps.fire === undefined)
                        return writeJson(res, 503, { error: 'reminders unavailable' });

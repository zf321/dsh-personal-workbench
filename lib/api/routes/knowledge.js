...
export function makeKnowledgeRoutes(db) {
    return [
        {
            kind: 'prefix',
            ...
            handler: async (req, res) => {
                ...
                if (segments.length === 1 && segments[0] === 'read-local-file') {
                    ...
                    try {
                        ...
                        if (boundary.mode === 'workspace' && !isInside(boundary.root, filePath))
                            return writeJson(res, 403, { error: 'path is outside your workspace' });
                        const info = await stat(filePath);
                        if (!info.isFile())
                            return writeJson(res, 400, { error: 'path is not a file' });
                ...
                if (segments.length === 0) {
                    if (method === 'GET') {
                        ...
                        if (kindCode !== undefined && getDictionary(db, 'knowledge_kind', kindCode) === undefined)
                            return writeJson(res, 400, { error: 'unknown knowledge_kind' });
                    ...
                    if (method === 'POST') {
                        ...
                        try {
                            ...
                            if (fileLink === undefined)
                                throw new Error('fileLink must be a string or null');
                ...
                if (method === 'PATCH' && segments.length === 1) {
                    ...
                    if (typeof body.contentMd === 'string')
                        patch.contentMd = body.contentMd;
                    if (typeof body.kindCode === 'string') {
                        requireCode(db, 'knowledge_kind', body.kindCode, 'kindCode');
                        patch.kindCode = body.kindCode;
                    }
                    if (Array.isArray(body.tags))
                        patch.tags = body.tags.filter((tag) => typeof tag === 'string').slice(0, 20);
                    if ('sourceTaskId' in body)
                        patch.sourceTaskId = typeof body.sourceTaskId === 'string' ? body.sourceTaskId : null;
                    if ('sourceReviewId' in body)
                        patch.sourceReviewId = typeof body.sourceReviewId === 'string' ? body.sourceReviewId : null;
                    if ('fileLink' in body)
                        patch.fileLink = typeof body.fileLink === 'string' ? body.fileLink : null;
                    const entry = updateKnowledge(db, id, patch);
                    if (entry === undefined)
                        return writeJson(res, 404, { error: 'knowledge not found' });

import { createMcpServer, deleteMcpServer, getMcpServer, listMcpServers, recordMcpStatus, recordMcpTools, updateMcpServer, } from '../../db/repo.js';
...
async function probeAndRecord(db, id) {
...
export function makeMcpRoutes(db) {
    return [
        {
            kind: 'prefix',
            path: MCP_PREFIX,
            handler: async (req, res) => {
                ...
                if (segments.length >= 2 && segments[0] === 'servers') {
                    ...
                    const row = getMcpServer(db, id);
                    if (row === undefined)
                        return writeJson(res, 404, { error: 'mcp server not found' });

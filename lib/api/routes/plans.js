...
export function makePlanRoutes(db) {
    return [
        {
            kind: 'prefix',
            path: PLANS_PREFIX,
            handler: async (req, res) => {
                ...
                if (segments.length === 0 && method === 'GET') {
                    ...
                    if (!PERIOD_DATE_RE.test(planDate))
                        return writeJson(res, 400, { error: 'date must be YYYY-MM-DD' });
                    const plan = getDailyPlan(db, planDate);
                    return writeJson(res, 200, { ok: true, plan: plan ?? null });
                }
                if (segments.length === 1 && method === 'PUT') {
                    if (!/^\d{4}-\d{2}-\d{2}$/.test(segments[0]))
                        return writeJson(res, 400, { error: 'invalid plan date' });
                    if (segments[0] < localDateString())
                        return writeJson(res, 400, { error: 'past plan is read-only' });

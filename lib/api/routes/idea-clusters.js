import { createIdeaCluster, deleteIdeaCluster, getIdeaCluster, listIdeaClusters } from '../../db/repo.js';
...
export function makeIdeaClusterRoutes(db) {
    return [
        {
            kind: 'prefix',
            ...
            handler: async (req, res) => {
                ...
                if (segments.length === 0 && method === 'POST') {
                    ...
                    const ideaIds = Array.isArray(body.ideaIds) ? body.ideaIds.filter((id) => typeof id === 'string') : [];
                    return writeJson(res, 201, { ok: true, cluster: createIdeaCluster(db, { title, summaryMd: typeof body.summaryMd === 'string' ? body.summaryMd : '', tags: Array.isArray(body.tags) ? body.tags.filter((tag) => typeof tag === 'string') : [], ideaIds }) });
                }

...
export function proposeIdeaClustersTool(db) {
    return defineTool({
        ...
        async execute(args, exec) {
            const raw = Array.isArray(args.clusters) ? args.clusters : [];
            if (raw.length === 0)
...
export function submitIdeaTasksTool(db) {
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            const sourceIdeaIds = Array.isArray(args.source_idea_ids) ? args.source_idea_ids.filter((id) => typeof id === 'string') : [];

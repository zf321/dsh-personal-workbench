...
const EMPTY: readonly TenantUser[] = []

let cache: { key: string; users: readonly TenantUser[] } | undefined

/** 宿主多租户插件的状态文件路径（env 可覆盖，便于测试与定制部署）。 */
export function tenantStateFilePath(): string {
...
export function listTenantUsers(): readonly TenantUser[] {
  ...
  if (cache !== undefined && cache.key === key) return cache.users
  try {
    const raw = JSON.parse(readFileSync(file, 'utf8')) as { users?: Record<string, unknown> }
    const users: TenantUser[] = []
    for (const value of Object.values(raw.users ?? {})) {
      if (typeof value !== 'object' || value === null) continue
      const row = value as Record<string, unknown>
      if (typeof row.slug !== 'string' || !isSafeUserSlug(row.slug)) continue
      if (typeof row.workspacePath !== 'string' || row.workspacePath === '') continue
...
export function resolveTenantUserByCwd(cwd: string | undefined): TenantUser | undefined {
  if (cwd === undefined || cwd.trim() === '') return undefined
  for (const user of listTenantUsers()) {
    if (isInside(user.workspacePath, cwd)) return user
  }
  return undefined
}

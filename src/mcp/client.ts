...
function withClient<T>(target: McpServerTarget, fn: (client: Client) => Promise<T>): Promise<T> {
...
export async function probeMcpServer(target: McpServerTarget): Promise<McpProbeResult> {
  try {
...
function summarizeCallToolResult(result: unknown): string {
  const parts: string[] = []
  ...
  if (Array.isArray(content)) {
    for (const block of content) {
      ...
      else if (typeof item.type === 'string') parts.push(`[${item.type} 内容]`)
    }
  }

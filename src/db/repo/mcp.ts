...
export function normalizeMcpServerName(raw: string): string {
  ...
  if (name.length > NAME_MAX) throw new Error(`name too long (max ${NAME_MAX} chars)`)
  return name
}
...
export function normalizeMcpServerUrl(raw: string): string {
  ...
  let url: URL
  try { url = new URL(trimmed) } catch { throw new Error('url is not a valid URL') }
...
export function normalizeMcpHeaders(raw: unknown): Record<string, string> {
  ...
  if (entries.length > HEADERS_MAX) throw new Error(`too many headers (max ${HEADERS_MAX})`)
  const out: Record<string, string> = {}
  for (const [key, value] of entries) {
    const name = key.trim()
    if (!HEADER_NAME_RE.test(name)) throw new Error(`invalid header name: ${key}`)
    if (typeof value !== 'string') throw new Error(`header ${name} must be a string`)
    if (value.length > 4096) throw new Error(`header ${name} is too long`)
    out[name] = value
  }
  return out
}

export function clampMcpTimeoutMs(raw: unknown): number {
  if (typeof raw !== 'number' || !Number.isFinite(raw)) return MCP_TIMEOUT_DEFAULT_MS
...
function parseTools(json: string): McpToolInfo[] {
  try {
    ...
    return parsed
      .filter((item): item is McpToolInfo =>
        typeof item === 'object' && item !== null && typeof (item as McpToolInfo).name === 'string')
...
function parseMcpServer(row: RawMcpServerRow | undefined): McpServerRow | undefined {
...
function assertNameFree(db: DatabaseSync, name: string, exceptId?: string): void {
  const row = db.prepare('SELECT id FROM mcp_servers WHERE name = ?').get(name) as { id: string } | undefined
  if (row !== undefined && row.id !== exceptId) throw new Error(`a MCP server named "${name}" already exists`)
}

export function createMcpServer(db: DatabaseSync, input: McpServerInput, at = nowIso()): McpServerRow {
  ...
  `).run(
    id, name, url, input.transport === 'sse' ? 'sse' : 'http', JSON.stringify(headers),
    input.enabled === false ? 0 : 1, Math.round(input.sortOrder ?? 0), clampMcpTimeoutMs(input.timeoutMs), at, at,
  )
  return getMcpServer(db, id)!
}

export function getMcpServer(db: DatabaseSync, id: string): McpServerRow | undefined {
  return parseMcpServer(db.prepare('SELECT * FROM mcp_servers WHERE id = ?').get(id) as RawMcpServerRow | undefined)
}

export function getMcpServerByName(db: DatabaseSync, name: string): McpServerRow | undefined {
  return parseMcpServer(db.prepare('SELECT * FROM mcp_servers WHERE name = ?').get(name.trim()) as RawMcpServerRow | undefined)
}

export function listMcpServers(db: DatabaseSync): McpServerRow[] {
  const rows = db.prepare('SELECT * FROM mcp_servers ORDER BY sort_order ASC, created_at ASC').all() as unknown as RawMcpServerRow[]
  return rows.map((row) => parseMcpServer(row)).filter((row): row is McpServerRow => row !== undefined)
}

export function updateMcpServer(db: DatabaseSync, id: string, patch: McpServerPatch, at = nowIso()): McpServerRow | undefined {
  ...
  if (name !== before.name) assertNameFree(db, name, id)
...
export function recordMcpTools(db: DatabaseSync, id: string, tools: McpToolInfo[], at = nowIso()): void {
  db.prepare(`
...
export function recordMcpStatus(db: DatabaseSync, id: string, status: 'ok' | 'error', error: string | null, at = nowIso()): void {

...
export class WorkbenchDbPool {
  private readonly store = new AsyncLocalStorage<DatabaseSync>()
  private readonly userDbs = new Map<string, DatabaseSync>()
  private readonly dataDir: string
  private readonly defaultDb: DatabaseSync
  ...
  handle(): DatabaseSync {
    const pool = this
    return {
      prepare: (sql: string) => pool.current().prepare(sql),
      exec: (sql: string) => pool.current().exec(sql),
  ...
  private forUser(slug: string): DatabaseSync {
    if (!isSafeUserSlug(slug)) throw new Error(`unsafe user slug: ${slug}`)
  ...
  private openAt(dbPath: string): DatabaseSync {
...
let activePool: WorkbenchDbPool | undefined

/** 插件 apply 时绑定进程内活动池（单实例；新池覆盖旧池）。 */
export function bindWorkbenchDbPool(pool: WorkbenchDbPool): void {
  activePool = pool
}

/** 鉴权解析出用户后进入其库上下文；未绑定池（如单测）时空操作。 */
export function enterWorkbenchUserDb(slug: string): void {
 ...
 * 未绑定池（如单测）时直接执行 fn。
 */
export function runWithWorkbenchUserDb<T>(slug: string, fn: () => T): T {

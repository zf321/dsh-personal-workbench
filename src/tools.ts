...
export function mcpListTool(db: DatabaseSync) {
  return defineTool({
    name: 'workbench_mcp_list',
    description:
      '个人工作台 MCP 工具：列出当前用户在「工作台 → 设置 → MCP 服务」配置的个人 MCP 服务及其可用工具。' +
    ...
    async execute(args: Record<string, unknown>) {
      ...
      if (args.refresh === true) {
        ...
        for (const row of rows.filter((item) => item.enabled === 1).slice(0, 5)) {
          ...
          if (result.ok) recordMcpTools(db, row.id, result.tools)
          else recordMcpStatus(db, row.id, 'error', result.error ?? 'probe failed')
        }
        rows = listMcpServers(db).filter((row) => ids.has(row.id))
      }
      if (rows.length === 0) return '错误：当前用户未配置个人 MCP 服务（可在 工作台 → 设置 → MCP 服务 添加）。'
      const lines: string[] = [`个人 MCP 服务 ${rows.length} 个：`]
      rows.forEach((row, index) => {
        ...
        if (row.tools.length === 0 && row.enabled === 1) lines.push('   （工具清单为空：可在界面点「探测」刷新）')
      })
...
export function mcpCallTool(db: DatabaseSync) {
  return defineTool({
    name: 'workbench_mcp_call',
    description:
      '个人工作台 MCP 工具：调用当前用户配置的个人 MCP 服务上的工具。' +
    ...
    async execute(args: Record<string, unknown>) {
      const serverArg = str(args.server)
      if (serverArg === undefined) return '错误：server 必填'
      const toolName = str(args.tool)
      if (toolName === undefined) return '错误：tool 必填'
